...
func (m *postgresDBRepo) Authenticate(email, unhashedPassword string) (int, string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m *postgresDBRepo) DeleteReservation(id int) error {
	...
	query := `
				delete from reservations wher id = $1
	`
...
func (m *postgresDBRepo) UpdateProcessedForReservation(id, proccessed int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
